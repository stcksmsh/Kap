...
enum class WeightUnits(val symbol:String,val kgs: Float, val decimals: Int, val fullName: String) {
    ...
    fun convertKilosToString(value: Float): String {
        ...
        val roundedValue = BigDecimal(scaledValue.toDouble()).setScale(decimals, RoundingMode.HALF_UP)
        return roundedValue.stripTrailingZeros().toPlainString()
    }
}

enum class VolumeUnits(val symbol: String, val milliliters: Float, val decimals: Int, val fullName: String) {
